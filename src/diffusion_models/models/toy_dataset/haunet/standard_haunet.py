...
def objective(trial: optuna.Trial) -> float:
    ...
    initial_features = trial.suggest_categorical("initial_features", [4, 8, 16])
    ...
    num_cc_heads = trial.suggest_categorical("num_cc_heads", [2, 4, 8])
    num_cc_layers = trial.suggest_int("num_cc_layers", 1, 3)
...
if __name__ == "__main__":
    mlflow.set_experiment("standard_haunet")
    ...
    with mlflow.start_run(run_name="standard_haunet") as run:
        ...
        model = HAUNet(
            input_channels=3,
            initial_features=study.best_params["initial_features"],
            ...
            num_cc_heads=study.best_params["num_cc_heads"],
            num_cc_layers=study.best_params["num_cc_layers"],
        )
        trainer = CFGTrainer(
            path=path,
            model=model,
        ...
        # Log the best model
        model_info = mlflow.pytorch.log_model(model, name="best_haunet", run_id=run_id)
