...
from diffusion_models.architectures.cbam_unet import CBAMUNet
...
cfg = TrainingConfig(
    ...
    channels=9,
    sequence_length=128,
    experiment_name="cbam_unet_uci_har_v2",
    model_name="cbam_unet_uci_har",
    evaluator="ucihar",
)
...
def suggest_params(trial: optuna.Trial) -> dict:
    return {
        ...
        "cbam_reduction_ratio": trial.suggest_categorical(
            "cbam_reduction_ratio", [2, 4, 8]
        ),
        "cbam_kernel_size": trial.suggest_categorical("cbam_kernel_size", [3, 7, 11]),
        "learning_rate": trial.suggest_categorical("learning_rate", [1e-4, 5e-4, 1e-3]),
    }


def build_model(hyperparams: dict) -> CBAMUNet:
    return CBAMUNet(
        ...
        cbam_reduction_ratio=hyperparams["cbam_reduction_ratio"],
        cbam_kernel_size=hyperparams["cbam_kernel_size"],
    )
...
if __name__ == "__main__":
    ...
    mlflow.set_experiment_tag("model_family", "cbam_unet")
