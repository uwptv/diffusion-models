...
def objective(trial: optuna.Trial) -> float:
    ...
    net = MBConvUNet(
        ...
        cond_dim=cond_dim,
        num_mbconv_layers=num_mbconv_layers,
...
if __name__ == "__main__":
    mlflow.set_experiment("mbconv_unet_wisdm")

    study = optuna.create_study(
        direction="minimize",
        pruner=MedianPruner(
            n_startup_trials=10, n_warmup_steps=50, interval_steps=10, n_min_trials=5
        ),
    )
    ...
    with mlflow.start_run(run_name="mbconv_unet_wisdm") as run:
        ...
        model = MBConvUNet(
            ...
            num_mbconv_layers=study.best_params["num_mbconv_layers"],
        ...
        model_info = mlflow.pytorch.log_model(
            model, name="best_mbconv_unet", run_id=run_id
        )

        # Register the best model as an MLflow model version
        model_info = mlflow.register_model(
            model_uri=model_info.model_uri,
            name="BestMBConvUNetWISDM",
        )
