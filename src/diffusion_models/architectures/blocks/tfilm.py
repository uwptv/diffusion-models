...
class TFiLMTransformer(nn.Module):
    def __init__(self, num_blocks: int, channels: int, num_heads: int = 8, num_layers: int = 6):
        super().__init__()
        self.num_blocks = num_blocks
...
class TFiLMEncoder(nn.Module):
    def __init__(self, channels_in: int, channels_out: int, num_residual_layers: int, num_tfilm_blocks: int, cond_dim: int, use_transformer: bool = False):
        super().__init__()
        ...
        if use_transformer:
            self.tfilm = TFiLMTransformer(num_blocks=num_tfilm_blocks, channels=channels_out, num_heads=8, num_layers=6)
        else:
...
class TFiLMMidcoder(nn.Module):
    def __init__(self, channels: int, num_residual_layers: int, num_tfilm_blocks: int, cond_dim: int, use_transformer: bool = False):
        super().__init__()
        self.res_blocks = nn.ModuleList([
        ...
        self.activation = nn.ReLU()
        if use_transformer:
            self.tfilm = TFiLMTransformer(num_blocks=num_tfilm_blocks, channels=channels, num_heads=8, num_layers=6)
        else:
