...
from .utils.sizes import model_size_b, MiB
...
class CFGTrainer(Trainer):
    def __init__(self, path: GaussianConditionalProbabilityPath, model: ConditionalVectorField, eta: float, null_label, **kwargs):
    ...
    def get_train_loss(self, batch_size: int) -> torch.Tensor:
        ...
        z, y = self.path.sample_conditioning_variable(batch_size) # z shape (batch_size, c, x_dim), y shape (batch_size, 1)
        ...
        t = torch.rand((batch_size, )+ (1, ) * (z.ndim - 1)).to(z.device) # (batch_size, 1, x_dim)
