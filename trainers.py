...
class WaveTrainer(Trainer):
    ...
    def get_train_loss(self, batch_size: int) -> torch.Tensor:
        # Step 1: Sample z,y from p_data
        z, y = self.path.sample_conditioning_variable(batch_size) # z shape (batch_size, 3, seq_len), y shape (batch_size, 1)
        ...
        # Step 3: Sample t and x
        t = torch.rand(batch_size, 1, 1).to(z.device) # (batch_size, 1, 1)
        x = self.path.sample_conditional_path(z, t) # (batch_size, 3, seq_len)
